...
fn main() -> Result<(), ImageDataErrors> {
    ...
    let mut result = FloatingImage::new(image_1.width(), image_2.height(), args.output);
...
fn get_smallest_dimension(dim_1: (u32, u32), dim_2: (u32, u32)) -> (u32, u32) {
    let pix_1: u32 = dim_1.0 * dim_2.0;
    let pix_2: u32 = dim_1.1 * dim_2.1;
    return if pix_1 < pix_2 { dim_1 } else { dim_2 };
}

fn standardise_size(image_1: DynamicImage, image_2: DynamicImage) -> (DynamicImage, DynamicImage) {
    let (width, height): (u32, u32) =
        get_smallest_dimension(image_1.dimensions(), image_2.dimensions());
